...
sender_email = "rohannmahajan@gmail.com"     # sender's email
receiver_email = "ronny@gmail.com"             # receiver's email
...
# Load defects.csv and test_cases.csv
try:
...
# Build documents from defects.csv using the columns "Description" and "Solution" (and "Module")
docs = []
...
# Function to fetch test cases from CSV (no classification)
...
# Helper to parse generated test case into fields
...
# Core node: Validate or generate test cases
def validate_or_generate_test_cases(state: AgentState) -> dict:
    try:
        error_message = state["input"]
        solution = None
        module = None
        # Try to retrieve a defect from the vector store
        ...
        # Try to fetch test cases from CSV if module info is available
        csv_test_cases = []
        if module:
        ...
        if csv_test_cases:
            ...
            test_cases_text = "\n\n".join(format_tc(tc) for tc in csv_test_cases)
            if not solution:
                gen_prompt = """
...
# Function to send email with the final solution
...
# Main function: execute the agent and send an email with the output.
def main():
